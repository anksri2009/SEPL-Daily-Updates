...
def get_all_user_data():
    ...
    conn = get_db_connection()
    df = pd.read_sql_query('SELECT user_identifier as "User", user_data as "Stored Data", timestamp as "Timestamp" FROM user_records ORDER BY timestamp DESC', conn)
    conn.close()
    return df
...
def get_personal_user_data(user_id):
    ...
    conn = get_db_connection()
    df = pd.read_sql_query('SELECT user_data as "Stored Data", timestamp as "Timestamp" FROM user_records WHERE user_identifier=? ORDER BY timestamp DESC', conn, params=(user_id,))
    conn.close()
    return df

# ==========================================
# 3. CALLBACKS (STATE MUTATION)
# ==========================================
...
def team_view(user_id, full_name):
    ...
    # Use tabs to organize the original task manager and the new data storage
    tab_tasks, tab_storage = st.tabs(["📋 Task Manager", "💾 Personal Data Storage"])
            ...
            with st.expander("➕ Create a New Task", expanded=False):
                ...
                if st.button("Add Task", use_container_width=True):
                    if p_name and t_name:
                        ...
                        st.warning("Please provide both a project name and a task name.")
    ...
    # --- DATA STORAGE TAB ---
    with tab_storage:
        st.markdown("### Secure HDD Notes & Data Storage")
        with st.form("user_data_form"):
            user_notes = st.text_area("Enter any data, links, or notes you want permanently stored:")
            submitted = st.form_submit_button("Save to HDD")
        ...
        if not historical_data.empty:
            ...
            st.caption(f"Data stored locally at: `{os.path.abspath('sabhiv_enterprise.db')}`")
        else:
...
def manager_view():
    ...
    # Use tabs to keep the manager view organized
    tab_dash, tab_storage = st.tabs(["📊 Global Dashboard", "💾 All User HDD Data"])
    ...
    # --- MANAGER DATA VIEW ---
    with tab_storage:
        st.markdown("### Global HDD Stored Records")
        st.write("This tab displays all data permanently logged to the local drive by any user.")
        
        all_data = get_all_user_data()
        if not all_data.empty:
            st.dataframe(all_data, use_container_width=True, hide_index=True)
            st.caption(f"Database active at: `{os.path.abspath('sabhiv_enterprise.db')}`")
        else:
